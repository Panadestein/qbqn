...
function parseBQN(str) {
    ...
    const digC = '5'; const dig = "0123456789π∞"; const digS = dig+"¯";
	...
	if (digS.includes(c)) {
	    res[i] = digC; i++;
	    while(dig.includes(str[i]) || "_eEiI".includes(str[i]) || (str[i]=='.' && digS.includes(str[i+1]))) i++;
	    continue;
	}
...
[...document.getElementsByTagName("code"),...document.getElementsByTagName("pre")].forEach(c => {
    let str = [...c.innerText];
    c.innerHTML = colorCode(str, parseBQN(str), 'B');
})

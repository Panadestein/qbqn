...
#+begin_src bash :exports results :tangle no :results raw :wrap example
  hyperfine --runs 1 'cbqn -f ./perf/q.bqn' 'sbcl --script ./perf/q.lisp'
#+end_src

#+RESULTS:
#+begin_example
Benchmark 1: cbqn -f ./perf/q.bqn
  Time (abs ≡):          8.6 ms               [User: 5.8 ms, System: 2.7 ms]
 
Benchmark 2: sbcl --script ./perf/q.lisp
  Time (abs ≡):         8.727 s               [User: 8.760 s, System: 0.086 s]
 
Summary
  cbqn -f ./perf/q.bqn ran
 1016.22 times faster than sbcl --script ./perf/q.lisp
#+end_example

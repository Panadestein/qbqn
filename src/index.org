...
We define the src_bqn[:exports code]{prog} string with the full program's content. We used:
...
BQN is also fast. While the interpreter is not particularly optimized, here is a full program's profiling and a comparison
with the equivalent Common Lisp code:
...
(REPL): 27637 samples:
    13│Q ← {𝕊qb‿sc‿rnd:
     2│  psi ← {(1⌾⊑⋈⊢)𝕩⥊0} 2⋆qb
  2618│  M‿K‿D ← ⟨+˝∘×⎉1‿∞ _cp, {1≡𝕩 ? 𝕨; 1≡𝕨 ? 𝕩; 𝕨(∾⊣×·<⊢)_cp𝕩}, {⌊2⋆⁼≠⊑𝕩}⟩
    65│  L ← {E ← {0=𝕨 ? 1; K⍟(𝕨-1)˜ 𝕩} ⋄ K´ ⟨(qb-𝕨+D𝕩) E g.id, 𝕩, 𝕨 E g.id⟩}
    24│  T ← ∾↕∘≠{a←𝕩 ⋄ i←𝕨{𝕩⊑a}•_while_{𝕩<𝕨}𝕨⊑a ⋄ 𝕨<◶⟨⟩‿{(⊢∾1⊸↓∘⌽)𝕨+↕𝕩-𝕨}i}¨<
     4│  A ← {qs‿gs𝕊v:
    10│    1⊸=◶{𝕊𝕩:
    23│      ui‿pt ← ⟨0 L gs, T qs (⌽∘⊢∾¬∘∊/⊣)˜ ↕qb⟩
 24504│      v M˜ {0=≠𝕩 ? ui; {𝕩 M ui M 𝕨}´ (M˜´⋈M´) {𝕩 L g.swap}¨𝕩} pt
   371│    }‿(v M˜ gs L˜ ⊑qs) ≠qs}
     3│  »⊸<∨` 0> rnd -` >+○(×˜)˝ psi A´ ⌽sc
      │}
#+end_example

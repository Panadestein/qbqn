...
#+begin_src bqn :exports code :results none :tangle ./perf/q.bqn
  ...
  g ← {
    I ← (⊢⋈≢⥊⟜0⊢)¨
    id‿x‿h ⇐ (⌽‿⊢{𝕎𝕩}¨<=⌜˜↕2) ∾○I <-⌾(1‿1⊸⊑)2‿2⥊÷√2
    swap‿cnot ⇐ I ⟨1‿2, 2‿3⟩ {⌽⌾(𝕨⊸⊏)𝕩}¨ <=⌜˜↕4
...
Why BQN, you may ask? Its tacit and functional style coupled with support
for numerous combinators, which makes programming feel like solving a fun algebraic problem
rather than writing a lengthy manifesto. Try running the simulation in
